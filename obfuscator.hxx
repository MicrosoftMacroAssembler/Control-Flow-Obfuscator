...
#define OBF_STRINGIZE(x) #x
#define OBF_STRINGIZE_VALUE(x) OBF_STRINGIZE(x)
#define OBF_UNIQUE_ID __FILE__ ":" OBF_STRINGIZE_VALUE(__LINE__)

// Define your project root macro in your own source files
#ifdef MY_PROJECT_ROOT
    // Apply CFLOW only in your project sources
...
//                        ) else
#else
    // In external headers / outside your project, use normal syntax
#define OBF_IF(x) if(x)
#define OBF_FOR(x) for(x)
#define OBF_WHILE(x) while(x)
#define OBF_SWITCH(x) switch(x)
#define OBF_RETURN return
